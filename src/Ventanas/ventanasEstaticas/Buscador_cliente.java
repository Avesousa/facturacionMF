...
public class Buscador_cliente extends javax.swing.JFrame {
    ...
    private void crearCliente(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_crearCliente
        Formulario_cliente vc = new Formulario_cliente(this,v);
        vc.boton_crear_cliente.setVisible(true);
        vc.boton_editar_cliente.setVisible(false);
        vc.setVisible(true);
    ...
    private void editarCliente(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editarCliente
        ...
        if(fila != -1){
            Formulario_cliente vc = new Formulario_cliente(this,v);
            vc.documento_cliente.setText(this.tabla_buscador.getValueAt(fila,0).toString());
            vc.razonsocial_cliente.setText(this.tabla_buscador.getValueAt(fila,1).toString());
            vc.telefono_cliente.setText(this.tabla_buscador.getValueAt(fila,2).toString());
            vc.direccion_cliente.setText(this.tabla_buscador.getValueAt(fila,3).toString());
            vc.almacenDeDocumento = (int)this.tabla_buscador.getValueAt(fila, 0);
            vc.boton_crear_cliente.setVisible(false);
            vc.boton_editar_cliente.setVisible(true);
            vc.setVisible(true);
        }else {
            JOptionPane.showMessageDialog(null, "No haz seleccionado ningún cliente...");
        }
